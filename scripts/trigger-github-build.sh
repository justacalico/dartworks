...
# The dispatched run builds whatever the ref points at on GitHub, which can
# differ from the local commit if the mirrors have drifted.
PUSHED_SHA=$(git ls-remote --heads github "$REF" | awk '{print $1}')
