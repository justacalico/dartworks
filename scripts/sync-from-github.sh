...
GITHUB_REPO="justacalico/dartworks"
...
if [ -n "${GITLAB_TOKEN:-}" ]; then
  ...
  # ci.skip keeps the tag push from starting a pipeline that would just
  # re-watch the GitHub run this job was already triggered by.
  git push -f -o ci.skip gitlab-auth "$RELEASE_TAG"
fi
